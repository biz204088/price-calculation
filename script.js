function calculatePrice() {
    ...
    // Total Users Column Header
    var totalUsersHeader = document.createElement('th');
    totalUsersHeader.innerText = "Total Users";
    headerRow.appendChild(totalUsersHeader);

    // Product Tier Column Header
    var productTierHeader = document.createElement('th');
    productTierHeader.innerText = "Product Tier";
    headerRow.appendChild(productTierHeader);
    ...
    selectedPlans.forEach((plan, index) => {
        ...
        // Add Total Users cell for the first plan only, spanning 3 rows
        if (index === 0) {
            var totalUsersCell = row.insertCell();
            totalUsersCell.rowSpan = selectedPlans.length; // Spanning across all plan rows
            totalUsersCell.innerText = users.toString(); // Display total users
            totalUsersCell.style.textAlign = 'center'; // Center horizontally
            totalUsersCell.style.verticalAlign = 'middle'; // Center vertically
            totalUsersCell.style.fontWeight = 'bold'; // Ensure the font is bold
        }

        // Add Plan Name Cell with inline styling for consistency
        var planCell = row.insertCell();
        planCell.innerText = plan.charAt(0).toUpperCase() + plan.slice(1);
        planCell.style.textAlign = 'left'; // Align text to the left
        planCell.style.verticalAlign = 'middle'; // Center text vertically
        planCell.style.fontWeight = 'bold'; // Make plan name bold
        planCell.style.fontSize = '20px'; // Match font size of headers
        if (plan === 'basic') {
            planCell.style.paddingLeft = '20px'; // Additional padding for 'Basic' if needed
        }
